...
## Read census state population data, add column names, and make a substitution
#pop <- "http://www.census.gov/popest/data/state/asrh/2013/files/SCPRC-EST2013-18+POP-RES.csv"
#pop <- tbl_dt(fread(pop))
#setnames(pop, names(pop), 
#         c("sumlev", "region", "divison", "state", "stn", "pop2013", "pop18p2013", "pcntest18p"))
#pop <- mutate(pop, stn = gsub(" of ", " Of ", stn))

#pop <- read.csv(pop, stringsAsFactors=FALSE)
...
## Merge population data and gather categories into one variable
#all_states <- wapo_flat %>%
#    merge(pop, all.x = TRUE, by = "stn") %>%
#    select(-(sumlev:state), -(pop18p2013:pcntest18p)) %>%
